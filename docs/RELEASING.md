...
2. Merge the version commit to `main`, wait for Development build, then push a tag on that exact commit. For example, for the prepared 0.1.2 release:
...
3. The tag automatically triggers Release. A manual recovery run must select that same tag:
   ...
   ```sh
   gh workflow run release.yml --repo yxwuxuanl/fleet-webui --ref v0.1.2
   ```
